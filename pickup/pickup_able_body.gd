...
func pick_up(pick_up_by) -> void:
	...
	var marker3d: Marker3D = get_node("Marker3D")
	
	if marker3d:
		var transformDestiny = marker3d.transform
		transformDestiny.rotated(Vector3(0, 0, 1), deg_to_rad(180))
		snap_to = transformDestiny
